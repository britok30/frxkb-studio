...
function postJSON(id: string, body?: unknown): Request {
  return new Request(`http://localhost/api/projects/${id}/generate`, {
    method: "POST",
...
describe("POST /api/projects/[id]/generate", () => {
  it("enqueues the inngest event with the operator email and returns 202", async () => {
  ...
  it("forwards body options into the event payload", async () => {
    await POST(
      postJSON("p_1", { force: true, concurrency: 2, aspectRatio: "9:16" }),
      ctx("p_1")
    );
  ...
  it("returns 400 on bad JSON without enqueueing", async () => {
    const res = await POST(postJSON("p_1", "not json{{"), ctx("p_1"));
  ...
  it("returns 400 on bad aspect ratio without enqueueing", async () => {
    const res = await POST(postJSON("p_1", { aspectRatio: "21:9" }), ctx("p_1"));
    expect(res.status).toBe(400);
    expect(hoisted.send).not.toHaveBeenCalled();
  });
  ...
  it("returns 500 if inngest.send rejects", async () => {
    ...
    expect(body.error).toMatch(/inngest unreachable/i);
  });
});
